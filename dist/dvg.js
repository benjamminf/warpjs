...
(function (root, factory) {
	...
	function interpolate(p, t) {
		...
		while (orders.length < p.length) {
			...
			for (i = 1; i < q.length; i++) {
				...
				r.push([q0[0] + (q1[0] - q0[0]) * t, q0[1] + (q1[1] - q0[1]) * t]);
			}
	...
	normalize.path = function (path, smoothLines) {
		...
		for (i = 0; i < segments.numberOfItems; i++) {
			...
			if (/[MLHVCSQTA]/.test(type)) {
				...
				switch (type) {
					...
					case 'a':
						segment = path.createSVGPathSegArcAbs(x, y, segment.r1, segment.r2, segment.angle, segment.largeArcFlag, segment.sweepFlag);
						break;
		...
		for (i = 0; i < segments.numberOfItems; i++) {
			...
			if (segment.pathSegTypeAsLetter === 'A') {
				...
				var arcSegs = this.arc(startPoint.x, startPoint.y, segment.r1, segment.r2, segment.angle, segment.largeArcFlag, segment.sweepFlag, segment.x, segment.y);
		...
		if (smoothLines) {
			...
			for (i = 0; i < segments.numberOfItems; i++) {
				...
				if (segment.pathSegTypeAsLetter === 'L') {
					...
					segments.replaceItem(path.createSVGPathSegCurvetoQuadraticAbs(segment.x, segment.y, (startPoint.x + segment.x) / 2, (startPoint.y + segment.y) / 2), i);
				}
			}
		}
	};
  ...
  */
	normalize.createLineSegment = function (path, points) {
		switch (points.length) {
			...
			case 2:
				return path.createSVGPathSegCurvetoQuadraticAbs(points[0][0], points[0][1], points[1][0], points[1][1]);
				break;
				break;

			case 3:
				return path.createSVGPathSegCurvetoCubicAbs(points[0][0], points[0][1], points[1][0], points[1][1], points[2][0], points[2][1]);
				break;
		}
  ...
  */
	normalize.arc = function (sx, sy, rx, ry, angle, large, sweep, ex, ey) {
		...
		for (var i = 0; i < numSegs; i++) {
			...
			segments.push([[epx, epy], [sx + dx, sy + dy], [epx + dxe, epy + dye]]);
  ...
  */
	normalize.convert.circle = function (circle) {
		...
		return SVG('path', {
			d: ['M', cx, cy - r, 'A', r, r, 0, 0, 1, cx + r, cy, 'A', r, r, 0, 0, 1, cx, cy + r, 'A', r, r, 0, 0, 1, cx - r, cy, 'A', r, r, 0, 0, 1, cx, cy - r, 'Z'].join(' ')
		});
	};
  ...
  */
	normalize.convert.ellipse = function (ellipse) {
		...
		return SVG('path', {
			d: ['M', cx, cy - ry, 'A', rx, ry, 0, 0, 1, cx + rx, cy, 'A', rx, ry, 0, 0, 1, cx, cy + ry, 'A', rx, ry, 0, 0, 1, cx - rx, cy, 'A', rx, ry, 0, 0, 1, cx, cy - ry, 'Z'].join(' ')
		});
	};
  ...
  */
	normalize.convert.rect = function (rect) {
		...
		if (rx > 0 || ry > 0) {
			...
			path.push('M', x + rx, y, 'H', x + width - rx, 'A', rx, ry, 0, 0, 1, x + width, y + ry, 'V', y + height - ry, 'A', rx, ry, 0, 0, 1, x + width - rx, y + height, 'H', x + rx, 'A', rx, ry, 0, 0, 1, x, y + height - ry, 'V', y + ry, 'A', rx, ry, 0, 0, 1, x + rx, y);
		} else {
			path.push('M', x, y, 'H', x + width, 'V', y + height, 'H', x, 'V', y);
		}
	...
	function Piece(points) {
		...
		if (pointStart.type() !== Point.NORMAL) throw new Error('First point must be of NORMAL type');

		if (pointEnd.type() !== Point.NORMAL) throw new Error('Last point must be of NORMAL type');

		if (pointControl1.type() !== Point.CONTROL) throw new Error('Second point must be of CONTROL type');

		if (pointControl2 && pointControl2.type() !== Point.CONTROL) throw new Error('Third point must be of CONTROL type');
		...
		for (var i = 0; i < segList.numberOfItems; i++) {
			...
			switch (type) {
				...
				case 'Q':
					...
					nextPoint = new FixedPoint(segItem.x, segItem.y);
					segment = new Segment(priorPoint, new FixedPoint(segItem.x1, segItem.y1, Point.CONTROL), nextPoint);
				...
				case 'C':
					...
					nextPoint = new FixedPoint(segItem.x, segItem.y);
					segment = new Segment(priorPoint, new FixedPoint(segItem.x1, segItem.y1, Point.CONTROL), new FixedPoint(segItem.x2, segItem.y2, Point.CONTROL), nextPoint);
	...
	fn.toString = function () {
		...
		for (var i = 0; i < this._segments.length; i++) {
			...
			s.push(segment.toString(!priorSegment || priorSegment.endPoint() !== segment.startPoint()));
